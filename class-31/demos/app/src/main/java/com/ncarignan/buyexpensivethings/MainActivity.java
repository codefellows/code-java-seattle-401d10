...
public class MainActivity extends AppCompatActivity implements SkateboardRecyclerViewAdapter.ClickOnSkateboardAble {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CartItem newCartItem = CartItem.builder()
                ...
                .build();
        Amplify.API.mutate(
                ModelMutation.create(newCartItem),
                response -> Log.i(TAG, "onCreate: banana made successfully"), //success
                response -> Log.i(TAG, response.toString()) // failure
        );

// TODO: link the resources
        Amplify.API.query(
                ModelQuery.list(CartItem.class),
                response -> {

                    List<CartItem> items = new ArrayList<>();
                    for(CartItem item : response.getData()){
                        items.add(item);
